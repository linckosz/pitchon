...
var app_layers_question_feedPage = function(param){
	...
	if(!wrapper_read_only){
		...
		Elem.find("[find=input_create], [find=input_cancel]").hover(
			//MouseIn
			function(){
				var textarea = $("#app_layers_question_add_icon").find("[find=input_textarea]");
	...
	app_application_bruno.add("app_layers_question", "question", function(){
		...
		for(var i in items){
			...
			if($('#models_question_'+item['id']).length==0){
				...
				if(wrapper_read_only){
					Elem.find("[find=delete]").addClass("display_none");
				} else {
...
var app_layers_question_grumble_1 = function(){
	if($("#app_layers_question_add_icon").length>0){
		...
		if($("#app_layers_question_add_icon").find(".app_layers_question_add_icon_img").length > 0){
