...
Filters.grayscaleBruno = function(pixels, segmentation) {
  var interval = 256 / segmentation;
  var output = Filters.createImageData(pixels.width, pixels.height);
  var dst = output.data;
  var d = pixels.data;
  for (var i=0; i<d.length; i+=4) {
    ...
    var b = d[i+2];
    var v = 0.3*r + 0.59*g + 0.11*b;
    dst[i] = dst[i+1] = dst[i+2] = Math.min(255, interval * Math.round( v / interval ));
    dst[i+3] = d[i+3];
  }
  return output;
};

Filters.colorcaleBruno = function(pixels, segmentation) {
  var interval = 256 / segmentation;
  var output = Filters.createImageData(pixels.width, pixels.height);
  var dst = output.data;
  var d = pixels.data;
  for (var i=0; i<d.length; i+=4) {
    dst[i]   = Math.min(255, interval * Math.round( d[i]   / interval ));
    dst[i+1] = Math.min(255, interval * Math.round( d[i+1] / interval ));
    dst[i+2] = Math.min(255, interval * Math.round( d[i+2] / interval ));
    dst[i+3] = d[i+3];
  }
  return output;
};
